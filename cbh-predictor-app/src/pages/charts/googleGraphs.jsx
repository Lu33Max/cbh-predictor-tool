...
function GetClicksAndImpressionsOverTime(dates, period) {
    ...
    useEffect(() => {
        if(dates.length > 0){
            ...
            const url = `${Constants.API_URL_GOOGLE_ENTRIES}/filter/true/${sort}/null`
...
function GetCustomAreaBump(terms, dates, period) {
    ...
    useEffect(() => {
        if(dates.length > 0 && terms.length > 0){
            ...
            const url = `${Constants.API_URL_GOOGLE_ENTRIES}/filter/true/null/null`
            getData(url, filter, setEntries)
        }
    },[terms, dates, period])
...
const GoogleChart = () => {
    ...
    useEffect(() => {
        var url = `${Constants.API_URL_GOOGLE_ENTRIES}/dates`
        getDates(url)
